...
let add_type_decl (signature : module_signature) (decl : Ir.type_decl) :
    module_signature =
    ...
    | Ir.Record record_fields ->
        ...
        let fields =
          List.fold_left
            (fun acc field ->
              let existing =
                match StringMap.find_opt field.Ir.field_name acc with
                | Some items -> items
                | None -> []
              in
              let item =
                {
                  field_parent;
                  field_typ = field.Ir.field_typ;
                  field_loc = field.Ir.field_loc;
                }
              in
    ...
    | Ir.Variant ctors ->
        ...
        let constructors =
          List.fold_left
            (fun acc ctor ->
              let existing =
                match StringMap.find_opt ctor.Ir.ctor_name acc with
                | Some items -> items
                | None -> []
              in
              let item =
                {
                  ctor_parent;
                  ctor_args = ctor.Ir.ctor_args;
                  ctor_loc = ctor.Ir.ctor_loc;
                }
              in
...
let add_value (signature : module_signature) ~(name : string)
    (value : value_info) : module_signature =
...
let lookup_module (env : t) (name : Syntax.Ast.qname) :
    (module_signature, string) result =
      ...
      | None -> (
          let candidates =
            ...
            |> List.filter_map (fun (_key, signature) ->
                if String.equal (short_name signature.module_name) requested
                then Some signature
                else None)
          in
...
let find_opened_modules (env : t) : module_signature list =
  List.filter_map
    (fun name -> StringMap.find_opt (module_key name) env.modules)
    env.opened
...
let lookup_value (env : t) (name : Syntax.Ast.qname) :
    (value_info, string) result =
  match List.rev name with
        ...
        | None ->
            let candidates =
              ...
              |> List.filter_map (fun signature ->
                  StringMap.find_opt short_name signature.values)
            in
...
let lookup_type_decl (env : t) (name : Syntax.Ast.qname) :
      ...
      if module_path = [] then
        ...
        | None ->
            let candidates =
              find_opened_modules env
              |> List.filter_map (fun signature ->
                  StringMap.find_opt short_name signature.types)
            in
        ...
        | None ->
            Error
              (Printf.sprintf "unbound type %s"
                 (Syntax.Ast.string_of_qname name)))

let lookup_constructor (env : t) (name : Syntax.Ast.qname) :
    (constructor_info, string) result =
  match List.rev name with
  ...
  | short_name :: rev_mod ->
      ...
      if module_path = [] then
        let local =
          match
            StringMap.find_opt short_name env.current_module.constructors
          with
          | Some items -> items
          | None -> []
        in
        ...
        let candidates =
          ...
          | None -> []
        in
        unique_or_error "constructor"
          (Syntax.Ast.string_of_qname name)
          candidates

let lookup_field (env : t) (name : string) : (field_info, string) result =
  let local =
    match StringMap.find_opt name env.current_module.fields with
    | Some items -> items
    | None -> []
  in
  if local <> [] then unique_or_error "field" name local
    ...
    let candidates =
      find_opened_modules env
      |> List.concat_map (fun signature ->
          match StringMap.find_opt name signature.fields with
          | Some items -> items
          | None -> [])
    in
