...
and ctor_spec = { ctor_id : string; ctor_arg_types : Ir.typ list }
...
let type_error loc fmt =
  Printf.ksprintf
    (fun message ->
      Error (Printf.sprintf "%s at %s" message (Loc.to_string loc)))
    fmt
...
let rec equal_typ left right =
  ...
  | Ir.TUnit, Ir.TUnit ->
      true
  | Ir.TList lhs, Ir.TList rhs | Ir.TOption lhs, Ir.TOption rhs ->
      equal_typ lhs rhs
  | Ir.TTuple lhs, Ir.TTuple rhs ->
      List.length lhs = List.length rhs && List.for_all2 equal_typ lhs rhs
  | Ir.TRecord lhs, Ir.TRecord rhs | Ir.TVariant lhs, Ir.TVariant rhs ->
      lhs = rhs
  | Ir.TFunc (lhs_params, lhs_result), Ir.TFunc (rhs_params, rhs_result) ->
      ...
      && List.for_all2
           (fun (lhs : Ir.param_type) (rhs : Ir.param_type) ->
             lhs.Ir.param_label = rhs.Ir.param_label
             && equal_typ lhs.Ir.param_typ rhs.Ir.param_typ)
...
let ensure_type expected actual loc =
  if equal_typ expected actual then Ok ()
    ...
    type_error loc "type mismatch: expected %s but got %s"
...
let primitive_equality_type = function
  ...
  | Ir.TList _ | Ir.TOption _ | Ir.TTuple _ | Ir.TRecord _ | Ir.TVariant _
  | Ir.TFunc _ ->
      false

let primitive_order_type = function
  | Ir.TInt | Ir.TString | Ir.TFloat -> true
  | Ir.TBool | Ir.TUnit | Ir.TList _ | Ir.TOption _ | Ir.TTuple _ | Ir.TRecord _
  | Ir.TVariant _ | Ir.TFunc _ ->
      false

let ensure_builtin_allowed predicate kind typ loc =
  if predicate typ then Ok ()
  else
    type_error loc "%s does not support values of type %s" kind
      (string_of_typ typ)

let make_builtin_fn loc name =
  { Ir.expr_loc = loc; expr_desc = Ir.EVar [ name ] }
...
let builtin_value_info name loc =
  ...
  let binary left right result =
    TFunc
      ( [
          { param_label = None; param_typ = left };
          { param_label = None; param_typ = right };
        ],
        result )
  in
  let unary input result =
    TFunc ([ { param_label = None; param_typ = input } ], result)
  in
  let value_typ =
    match name with
  ...
  in
  Option.map
    (fun value_typ -> { Env.value_typ; value_kind = Env.User; value_loc = loc })
    value_typ

let rec resolve_type (env : Env.t) (typ : Syntax.Ast.type_expr) :
    (Ir.typ, string) result =
  ...
  | Syntax.Ast.TEConstr (name, args) -> (
      ...
      | _ -> (
          ...
          | Ir.Record _ ->
              if args <> [] then
                type_error typ.type_loc
                  "record type %s does not take type arguments" name_string
              else Ok (Ir.TRecord decl.Ir.type_name)
          | Ir.Variant _ ->
              if args <> [] then
                type_error typ.type_loc
                  "variant type %s does not take type arguments" name_string
  ...
  | Syntax.Ast.TEArrow (param, result) -> (
      ...
      let param =
        { Ir.param_label = param.Syntax.Ast.param_label; param_typ }
      in
      match result with
      | Ir.TFunc (params, final_result) ->
          Ok (Ir.TFunc (param :: params, final_result))
...
let resolve_param env (param : Syntax.Ast.param) : (Ir.param, string) result =
  ...
  | None ->
      type_error param.Syntax.Ast.param_loc
...
let rec type_decl_fields env type_name =
  ...
  | _ ->
      Error
        (Printf.sprintf "%s is not a record type"
           (Syntax.Ast.string_of_qname type_name))
...
let rec type_decl_ctors env type_name =
  ...
  | _ ->
      Error
        (Printf.sprintf "%s is not a variant type"
           (Syntax.Ast.string_of_qname type_name))
...
let ctor_id_of_type typ name =
  ...
  | Ir.TVariant qname ->
      "variant:" ^ Syntax.Ast.string_of_qname qname ^ ":" ^ name
...
let constructors_of_type env typ =
  ...
  | TBool ->
      Ok
        [
          { ctor_id = ctor_id_of_type typ "true"; ctor_arg_types = [] };
          { ctor_id = ctor_id_of_type typ "false"; ctor_arg_types = [] };
        ]
  | TUnit -> Ok [ { ctor_id = ctor_id_of_type typ "()"; ctor_arg_types = [] } ]
  | TOption inner ->
      Ok
        [
          { ctor_id = ctor_id_of_type typ "None"; ctor_arg_types = [] };
          { ctor_id = ctor_id_of_type typ "Some"; ctor_arg_types = [ inner ] };
        ]
  | TList inner ->
      Ok
        [
          { ctor_id = ctor_id_of_type typ "[]"; ctor_arg_types = [] };
          {
            ctor_id = ctor_id_of_type typ "::";
            ctor_arg_types = [ inner; typ ];
          };
        ]
  | TTuple items ->
      Ok [ { ctor_id = ctor_id_of_type typ "tuple"; ctor_arg_types = items } ]
  | TRecord name ->
      let* fields = type_decl_fields env name in
      Ok
        [
          {
            ctor_id = ctor_id_of_type typ "record";
            ctor_arg_types = List.map (fun field -> field.Ir.field_typ) fields;
          };
        ]
  | TVariant name ->
      ...
      Ok
        (List.map
           (fun ctor ->
             {
               ctor_id = ctor_id_of_type typ ctor.Ir.ctor_name;
               ctor_arg_types = ctor.Ir.ctor_args;
             })
           ctors)
...
let literal_ctor typ literal =
  ...
  | Ir.TBool, Ir.LBool true ->
      Some { ctor_id = ctor_id_of_type typ "true"; ctor_arg_types = [] }
  | Ir.TBool, Ir.LBool false ->
      Some { ctor_id = ctor_id_of_type typ "false"; ctor_arg_types = [] }
  | Ir.TUnit, Ir.LUnit ->
      Some { ctor_id = ctor_id_of_type typ "()"; ctor_arg_types = [] }
  | _ -> None
  ...
  let rec loop seen acc = function
    ...
    | ((name, _) as binding) :: rest ->
        if StringSet.mem name seen then
          type_error loc "duplicate binding %s in pattern" name
...
let reorder_record_fields loc expected_fields actual_fields =
  ...
  let missing =
    List.filter (fun field -> lookup field.Ir.field_name = None) expected_fields
  in
  let extra =
    List.filter
      (fun (name, _) ->
        not
          (List.exists
             (fun field -> String.equal field.Ir.field_name name)
             expected_fields))
      actual_fields
  in
  match (missing, extra) with
  | field :: _, _ ->
      type_error loc "missing record field %s in pattern or expression"
        field.Ir.field_name
  ...
  | [], [] ->
      Ok
        (List.map
           (fun field ->
             (field.Ir.field_name, Option.get (lookup field.Ir.field_name)))
           expected_fields)

let rec check_pattern env expected_type (pattern : Syntax.Ast.pattern) :
    (checked_pattern, string) result =
      ...
      Ok
        {
          ir_pattern =
            {
              Ir.pattern_loc = pattern.pattern_loc;
              pattern_desc = Ir.PWildcard;
            };
          bindings = [];
          normal_pattern = NWild;
        }
  | Syntax.Ast.PVar name ->
      let value =
        {
          Env.value_typ = expected_type;
          value_kind = Env.User;
          value_loc = pattern.pattern_loc;
        }
      in
      Ok
        {
  ...
  | Syntax.Ast.PLiteral literal ->
      ...
      let normal_pattern =
        match literal_ctor expected_type literal with
        | Some ctor -> NCtor (ctor, [])
        | None -> NConst literal
      in
      Ok
        {
          ir_pattern =
            {
              Ir.pattern_loc = pattern.pattern_loc;
              pattern_desc = Ir.PLiteral literal;
            };
          bindings = [];
          normal_pattern;
        }
      ...
      | Ir.TTuple types when List.length items = List.length types ->
          ...
          let* bindings =
            List.fold_left
              (fun acc item ->
                let* acc = acc in
                merge_bindings acc item.bindings pattern.pattern_loc)
              (Ok []) checked
          in
          let ir_pattern =
            {
              Ir.pattern_loc = pattern.pattern_loc;
              pattern_desc =
                Ir.PTuple (List.map (fun item -> item.ir_pattern) checked);
            }
          in
          let normal_pattern =
            NCtor
              ( {
                  ctor_id = ctor_id_of_type expected_type "tuple";
                  ctor_arg_types = types;
                },
                List.map (fun item -> item.normal_pattern) checked )
          in
          Ok { ir_pattern; bindings; normal_pattern }
      | Ir.TTuple _ ->
          type_error pattern.pattern_loc "tuple arity mismatch in pattern"
      | _ ->
          type_error pattern.pattern_loc "expected %s to be a tuple in pattern"
            (string_of_typ expected_type))
  | Syntax.Ast.PRecord fields -> (
      match expected_type with
      | Ir.TRecord name ->
          let* expected_fields = type_decl_fields env name in
          let* ordered =
            reorder_record_fields pattern.pattern_loc expected_fields fields
          in
          let* checked =
            all
              (List.map2
                 (fun field (_name, item) ->
                   check_pattern env field.Ir.field_typ item)
          ...
          let ir_fields =
            List.map2
              (fun field item -> (field.Ir.field_name, item.ir_pattern))
              expected_fields checked
          in
          let ir_pattern =
            {
              Ir.pattern_loc = pattern.pattern_loc;
              pattern_desc = Ir.PRecord ir_fields;
            }
          in
          let normal_pattern =
            NCtor
              ( {
                  ctor_id = ctor_id_of_type expected_type "record";
                  ctor_arg_types =
                    List.map (fun field -> field.Ir.field_typ) expected_fields;
                },
                List.map (fun item -> item.normal_pattern) checked )
          in
          Ok { ir_pattern; bindings; normal_pattern }
      | _ ->
          type_error pattern.pattern_loc
            "record pattern used with non-record type %s"
            (string_of_typ expected_type))
  | Syntax.Ast.PConstruct (name, args) -> (
      ...
      | Ir.TOption inner -> (
          let ctor =
            ...
            | "None" ->
                Some
                  {
                    ctor_id = ctor_id_of_type expected_type "None";
                    ctor_arg_types = [];
                  }
            | "Some" ->
                Some
                  {
                    ctor_id = ctor_id_of_type expected_type "Some";
                    ctor_arg_types = [ inner ];
                  }
            | _ -> None
          in
          match ctor with
          | Some ctor ->
              let* checked_args =
                check_ctor_pattern_args env pattern.pattern_loc ctor args
              in
              let* bindings =
                flatten_bindings pattern.pattern_loc checked_args
              in
              Ok
                {
                  ir_pattern =
                    {
                      ...
                      pattern_desc =
                        Ir.PConstruct
                          ( name,
                            List.map (fun item -> item.ir_pattern) checked_args
                          );
                    };
                  bindings;
                  normal_pattern =
                    NCtor
                      ( ctor,
                        List.map (fun item -> item.normal_pattern) checked_args
                      );
                }
          | None ->
              type_error pattern.pattern_loc
                "constructor %s does not belong to %s" name_text
                (string_of_typ expected_type))
      | Ir.TList inner -> (
          let ctor =
            match name_text with
            | "[]" ->
                Some
                  {
                    ctor_id = ctor_id_of_type expected_type "[]";
                    ctor_arg_types = [];
                  }
            | "::" ->
                Some
                  {
                    ctor_id = ctor_id_of_type expected_type "::";
                    ctor_arg_types = [ inner; expected_type ];
                  }
            | _ -> None
          in
          match ctor with
      ...
      | Ir.TVariant _parent ->
          ...
          let* () =
            ensure_type expected_type ctor_info.Env.ctor_parent
              pattern.pattern_loc
          in
          let ctor =
            {
              ctor_id = ctor_id_of_type expected_type name_text;
              ctor_arg_types = ctor_info.Env.ctor_args;
            }
          in
          let* checked_args =
            check_ctor_pattern_args env pattern.pattern_loc ctor args
              ...
              ir_pattern =
                {
                  ...
                  pattern_desc =
                    Ir.PConstruct
                      (name, List.map (fun item -> item.ir_pattern) checked_args);
                };
              bindings;
              normal_pattern =
                NCtor
                  (ctor, List.map (fun item -> item.normal_pattern) checked_args);
            }
      | _ ->
          type_error pattern.pattern_loc
            "constructor pattern %s does not match %s" name_text
            (string_of_typ expected_type))
...
and check_ctor_pattern_args env loc ctor args =
  if List.length args <> List.length ctor.ctor_arg_types then
    type_error loc "constructor expects %d arguments but got %d"
      (List.length ctor.ctor_arg_types)
      (List.length args)
...
let rec specialize_ctor_matrix ctor matrix =
  List.filter_map
    (function
      ...
      | NCtor (head, args) :: rest when String.equal head.ctor_id ctor.ctor_id
        ->
          Some (args @ rest)
      | _ -> None)
    matrix
...
let rec useful env types matrix vector =
  ...
  | typ :: rest_types, pattern :: rest_patterns -> (
          ...
          if finite then
            ...
            let rec try_ctors = function
              ...
              | ctor :: tail ->
                  let* is_useful =
                    useful env
                      (ctor.ctor_arg_types @ rest_types)
      ...
      | NConst constant -> (
          ...
          | None ->
              if finite then
                Error "internal error: finite literal should map to constructor"
              else
                let specialized =
                  List.filter_map
                    (function
...
let rec infer_expr env ?expected ?(allow_effectful_call = false)
  ...
  | Syntax.Ast.ELiteral literal ->
      ...
      in
      Ok
        ( { Ir.expr_loc = expr.expr_loc; expr_desc = Ir.ELiteral literal },
          { inferred_type; effectful = false; callable_kind = None } )
  | Syntax.Ast.EVar name ->
      let value_result =
        match name with
        | [ single ] -> (
            match builtin_value_info single expr.expr_loc with
            | Some value -> Ok value
            | None -> Env.lookup_value env name)
      ...
      let* () =
        ...
        | false, (Env.Agent | Env.Skill) ->
            type_error expr.expr_loc
              "agents and skills must be fully applied and cannot be used as \
               values"
        | _ -> Ok ()
      in
      let* () =
        ...
        | None -> Ok ()
        | Some expected ->
            ensure_type expected value.Env.value_typ expr.expr_loc
      in
      Ok
        ( { Ir.expr_loc = expr.expr_loc; expr_desc = Ir.EVar name },
          {
            inferred_type = value.Env.value_typ;
            effectful = false;
            callable_kind = Some value.Env.value_kind;
          } )
  | Syntax.Ast.ETuple items ->
      let* checked = all (List.map (infer_expr env) items) in
      let inferred_type =
        Ir.TTuple (List.map (fun (_, info) -> info.inferred_type) checked)
      in
      let* () =
        match expected with
        | None -> Ok ()
      ...
      Ok
        ( {
            Ir.expr_loc = expr.expr_loc;
            expr_desc = Ir.ETuple (List.map fst checked);
          },
          {
            inferred_type;
            effectful = List.exists (fun (_, info) -> info.effectful) checked;
            callable_kind = None;
          } )
  ...
  | Syntax.Ast.EField (target, field_name) ->
      ...
      let* inferred_type =
        ...
        | Ir.TRecord name -> (
            ...
            match
              List.find_opt
                (fun field -> String.equal field.Ir.field_name field_name)
                fields
            with
            | Some field -> Ok field.Ir.field_typ
            | None ->
                type_error expr.expr_loc "record type %s has no field %s"
                  (Syntax.Ast.string_of_qname name)
                  field_name)
        | other ->
            type_error expr.expr_loc
              "field access requires a record value, got %s"
              (string_of_typ other)
      in
      let* () =
        match expected with
      ...
      Ok
        ( {
            Ir.expr_loc = expr.expr_loc;
            expr_desc = Ir.EField (target_ir, field_name);
          },
          {
            inferred_type;
            effectful = target_info.effectful;
            callable_kind = None;
          } )
  | Syntax.Ast.EConstruct (name, args) ->
      infer_construct env ?expected expr name args
  | Syntax.Ast.ELet (binding, body) -> infer_let env ?expected expr binding body
  | Syntax.Ast.ELetStar (binding, body) ->
      infer_let_star env ?expected expr binding body
  | Syntax.Ast.EIf (cond, then_branch, else_branch) ->
      ...
      let branch_expected =
        ...
        | None -> Some then_info.inferred_type
      in
      let* else_ir, else_info =
        infer_expr env ?expected:branch_expected else_branch
      in
      let inferred_type =
        match expected with
        | Some value -> value
        | None -> then_info.inferred_type
      in
      let* () =
        ensure_type inferred_type else_info.inferred_type else_branch.expr_loc
      in
      Ok
        ( {
            Ir.expr_loc = expr.expr_loc;
            expr_desc = Ir.EIf (cond_ir, then_ir, else_ir);
          },
          {
            inferred_type;
            effectful =
              cond_info.effectful || then_info.effectful || else_info.effectful;
            callable_kind = None;
          } )
  | Syntax.Ast.EMatch (scrutinee, cases) ->
      infer_match env ?expected expr scrutinee cases
  | Syntax.Ast.EApply (fn, args) ->
      infer_apply env ~allow_effectful_call ?expected expr fn args
  | Syntax.Ast.ELambda (params, body) ->
      infer_lambda env ?expected expr params body
...
and infer_record env ?expected expr fields =
  let parent_type =
    ...
    | Some other ->
        type_error expr.expr_loc "expected record expression, got %s"
          (string_of_typ other)
    | None -> (
        match fields with
  ...
  | Ir.TRecord name ->
      let* expected_fields = type_decl_fields env name in
      let* ordered =
        reorder_record_fields expr.expr_loc expected_fields fields
      in
      let* checked =
        all
          (List.map2
             (fun field (_name, value) ->
               infer_expr env ~expected:field.Ir.field_typ value)
             expected_fields ordered)
      in
      let ir_fields =
        List.map2
          (fun field (value, _) -> (field.Ir.field_name, value))
          expected_fields checked
      in
      Ok
        ( { Ir.expr_loc = expr.expr_loc; expr_desc = Ir.ERecord ir_fields },
          {
...
and infer_construct env ?expected expr name args =
  ...
  | "None" ->
      let option_type =
        ...
        | Some other ->
            type_error expr.expr_loc "None does not match %s"
              (string_of_typ other)
        | None ->
            type_error expr.expr_loc "cannot infer type of None without context"
      in
      let* inferred_type = option_type in
      if args <> [] then type_error expr.expr_loc "None does not take arguments"
      else
        Ok
          ( { Ir.expr_loc = expr.expr_loc; expr_desc = Ir.EConstruct (name, []) },
            { inferred_type; effectful = false; callable_kind = None } )
  | "Some" ->
      if List.length args <> 1 then
        type_error expr.expr_loc "Some expects one argument"
      else
        let expected_inner =
          match expected with
          | Some (Ir.TOption inner) -> Some inner
          | _ -> None
        in
        let* arg_ir, arg_info =
          infer_expr env ?expected:expected_inner (List.hd args)
        in
        ...
        Ok
          ( {
              Ir.expr_loc = expr.expr_loc;
              expr_desc = Ir.EConstruct (name, [ arg_ir ]);
            },
            {
              inferred_type;
              effectful = arg_info.effectful;
              callable_kind = None;
            } )
  | "[]" ->
      let list_type =
        ...
        | Some other ->
            type_error expr.expr_loc "[] does not match %s"
              (string_of_typ other)
        | None ->
            type_error expr.expr_loc "cannot infer type of [] without context"
      in
  ...
  | "::" ->
      if List.length args <> 2 then
        type_error expr.expr_loc ":: expects two arguments"
      else
        let expected_inner =
          match expected with Some (Ir.TList inner) -> Some inner | _ -> None
        in
        let expected_tail =
          match expected with Some list_type -> Some list_type | None -> None
        in
        let* head_ir, head_info =
          infer_expr env ?expected:expected_inner (List.nth args 0)
        in
        let* tail_ir, tail_info =
          infer_expr env ?expected:expected_tail (List.nth args 1)
        in
        let* () =
          ensure_type (Ir.TList head_info.inferred_type) tail_info.inferred_type
            expr.expr_loc
        in
        ...
        Ok
          ( {
              Ir.expr_loc = expr.expr_loc;
              expr_desc = Ir.EConstruct (name, [ head_ir; tail_ir ]);
            },
            {
              inferred_type;
              effectful = head_info.effectful || tail_info.effectful;
              callable_kind = None;
            } )
  | _ ->
      let* ctor_info = Env.lookup_constructor env name in
      ...
      let* () =
        match expected with
        | None -> Ok ()
        | Some expected -> ensure_type expected parent_type expr.expr_loc
      in
      if List.length args <> List.length ctor_info.Env.ctor_args then
        type_error expr.expr_loc "constructor %s expects %d arguments" name_text
          (List.length ctor_info.Env.ctor_args)
      else
        let* checked =
          all
            (List.map2
               (fun arg typ -> infer_expr env ~expected:typ arg)
               args ctor_info.Env.ctor_args)
        in
        Ok
          ( {
              Ir.expr_loc = expr.expr_loc;
              expr_desc = Ir.EConstruct (name, List.map fst checked);
            },
            {
              inferred_type = parent_type;
              effectful = List.exists (fun (_, info) -> info.effectful) checked;
...
and infer_builtin_apply env ?expected (expr : Syntax.Ast.expr) name args =
  let infer_unlabeled ?expected arg =
    if Option.is_some arg.Syntax.Ast.arg_label then
      ...
      Ok
        ( { Ir.arg_label = None; arg_value = arg_ir; arg_loc = arg.arg_loc },
          arg_info )
  in
  ...
  let expected_result =
    ...
    | Some (Ir.TFunc (expected_params, result)) ->
        if List.length expected_params <> List.length params then
          type_error expr.expr_loc "lambda arity mismatch"
        else
          let* _validated =
            all
              (List.map2
                 (fun (expected : Ir.param_type) (param : Ir.param) ->
                   if expected.Ir.param_label <> param.Ir.param_label then
                     ...
                     ensure_type expected.Ir.param_typ param.Ir.param_typ
                       param.Ir.param_loc)
                 expected_params params)
          in
          Ok (Some result)
    | Some other ->
        type_error expr.expr_loc "expected function type, got %s"
          (string_of_typ other)
    | None -> Ok None
  in
    ...
    Env.with_locals env
      (List.map
         (fun (param : Ir.param) ->
           ( param.Ir.param_name,
             {
               Env.value_typ = param.Ir.param_typ;
               value_kind = Env.User;
               value_loc = param.Ir.param_loc;
             } ))
         params)
  in
  let* body_ir, body_info =
    infer_expr body_env ?expected:expected_result body
  in
  let inferred_type =
    Ir.TFunc
      ( List.map
          (fun (param : Ir.param) ->
            {
              Ir.param_label = param.Ir.param_label;
              param_typ = param.Ir.param_typ;
            })
          params,
        body_info.inferred_type )
  in
  Ok
    ( { Ir.expr_loc = expr.expr_loc; expr_desc = Ir.ELambda (params, body_ir) },
      { inferred_type; effectful = false; callable_kind = Some Env.User } )
  ...
  | _ -> (
      ...
      | Ir.TFunc (params, result_type) ->
          if List.length args > List.length params then
                ...
                (List.map
                   (fun ((param : Ir.param_type), arg) ->
                     if arg.Syntax.Ast.arg_label <> param.Ir.param_label then
                       ...
                       let* arg_ir, arg_info =
                         infer_expr env ~expected:param.Ir.param_typ
                           arg.arg_value
                       in
                       Ok
                         ( {
                             Ir.arg_label = arg.arg_label;
                             arg_value = arg_ir;
                             arg_loc = arg.arg_loc;
                           },
                           arg_info ))
            ...
            in
            let args_effectful =
              List.exists (fun (_, info) -> info.effectful) checked_args
            in
            let result =
              match (fn_info.callable_kind, remaining) with
              | Some (Env.Agent | Env.Skill), _ :: _ ->
                  type_error expr.expr_loc
                    "agents and skills must be fully applied"
              | Some (Env.Agent | Env.Skill), [] when not allow_effectful_call
                ->
                  type_error expr.expr_loc
                    "effectful agent and skill calls must appear directly on \
                     the RHS of let*"
              | Some (Env.Agent | Env.Skill), [] -> Ok (result_type, true, None)
              | _, [] ->
                  Ok (result_type, args_effectful || fn_info.effectful, None)
              | _, remaining ->
                  Ok
                    ( Ir.TFunc (remaining, result_type),
                      args_effectful || fn_info.effectful,
                      Some Env.User )
            in
            ...
            let* () =
              ...
              | None -> Ok ()
              | Some expected ->
                  ensure_type expected inferred_type expr.expr_loc
            in
            Ok
              ( {
                  Ir.expr_loc = expr.expr_loc;
                  expr_desc = Ir.EApply (fn_ir, List.map fst checked_args);
                },
...
and infer_match env ?expected expr scrutinee cases =
  ...
  let* checked_cases =
    all
      (List.map
         (fun case ->
           let* checked_pattern =
             check_pattern env scrutinee_info.inferred_type
               case.Syntax.Ast.case_pattern
           in
           let env' = Env.with_locals env checked_pattern.bindings in
           let* body_ir, body_info =
             infer_expr env' ?expected case.Syntax.Ast.case_body
           in
           Ok
             ( {
                 Ir.case_pattern = checked_pattern.ir_pattern;
                 case_body = body_ir;
  ...
  in
  let* () =
    ensure_reachable env scrutinee_info.inferred_type
      (List.map (fun (_, _, pattern) -> pattern) checked_cases)
      expr.expr_loc
  in
  let* () =
    ensure_match_exhaustive env scrutinee_info.inferred_type
      (List.map (fun (_, _, pattern) -> pattern) checked_cases)
      expr.expr_loc
  in
  let result_type =
  ...
  let* _validated =
    all
      (List.map
         (fun (_case, info, _) ->
           ensure_type result_type info.inferred_type expr.expr_loc)
         checked_cases)
  in
  Ok
    ( {
        ...
        expr_desc =
          Ir.EMatch
            (scrutinee_ir, List.map (fun (case, _, _) -> case) checked_cases);
      },
      {
        inferred_type = result_type;
        effectful =
          scrutinee_info.effectful
          || List.exists (fun (_case, info, _) -> info.effectful) checked_cases;
        callable_kind = None;
      } )
...
and infer_let env ?expected expr binding body =
  ...
  Ok
    ( { Ir.expr_loc = expr.expr_loc; expr_desc = Ir.ELet (ir_binding, body_ir) },
      {
        inferred_type = body_info.inferred_type;
        effectful = body_info.effectful;
        callable_kind = body_info.callable_kind;
      } )
...
and infer_let_star env ?expected expr binding body =
  ...
  in
  if not value_info.effectful then
    type_error binding.let_star_loc
      "let* requires an effectful call on the right-hand side"
  else
    let env' =
      Env.with_local env binding.let_star_name
        {
          Env.value_typ = value_info.inferred_type;
          value_kind = Env.User;
          value_loc = binding.let_star_loc;
        }
    in
    let* body_ir, body_info = infer_expr env' ?expected body in
    Ok
      ( {
          ...
          expr_desc =
            Ir.ELetStar
              ( {
                  Ir.let_star_name = binding.let_star_name;
                  let_star_value = value_ir;
                  let_star_loc = binding.let_star_loc;
                },
                body_ir );
        },
        {
...
and check_binding env (binding : Syntax.Ast.binding) :
    (Ir.binding * Env.value_info * bool, string) result =
  let* binding_params =
    all (List.map (resolve_param env) binding.binding_params)
  in
  let declared_type =
          ...
          (List.map2
             (fun (declared : Ir.param_type) (actual : Ir.param) ->
               if declared.Ir.param_label <> actual.Ir.param_label then
                 ...
                 ensure_type declared.Ir.param_typ actual.Ir.param_typ
                   actual.Ir.param_loc)
  ...
  let* body_env, expected_result =
    ...
    | true, [], _ ->
        type_error binding.binding_loc "recursive values must be functions"
    | true, _ :: _, None ->
        type_error binding.binding_loc
          "recursive functions require an explicit type annotation"
    | true, _ :: _, Some (Ir.TFunc (declared_params, result)) ->
        ...
        let self_info =
          {
            Env.value_typ = func_type;
            value_kind = Env.User;
            value_loc = binding.binding_loc;
          }
        in
        let body_env =
          Env.with_local env binding.binding_name self_info |> fun env ->
          Env.with_locals env param_locals
        in
        Ok (body_env, Some result)
    | true, _ :: _, Some other ->
        type_error binding.binding_loc
          "recursive annotation must be a function type, got %s"
          (string_of_typ other)
    ...
    | false, _ :: _, Some other ->
        type_error binding.binding_loc "expected function annotation, got %s"
          (string_of_typ other)
    | false, _params, None -> Ok (Env.with_locals env param_locals, None)
  in
  let* body_ir, body_info =
    infer_expr body_env ?expected:expected_result binding.binding_body
  in
  let binding_type =
...
let check_callable env (callable : Syntax.Ast.callable_decl) :
    (Ir.callable_decl * Env.value_info, string) result =
  let* callable_params =
    all (List.map (resolve_param_type env) callable.callable_params)
  in
  ...
  let callable_kind =
    match callable.callable_kind with
    | Syntax.Ast.Agent -> Ir.Agent
    | Skill -> Ir.Skill
  in
  let ir_callable =
    {
  ...
  let value_kind =
    match callable.callable_kind with
    | Syntax.Ast.Agent -> Env.Agent
    | Skill -> Env.Skill
  in
  let value_typ = Ir.TFunc (callable_params, callable_return_type) in
  Ok
    ( ir_callable,
      { Env.value_typ; value_kind; value_loc = callable.callable_loc } )
...
let check_type_decl env module_name (decl : Syntax.Ast.type_decl) :
    (Ir.type_decl, string) result =
  let type_name = module_name @ [ decl.type_name ] in
  let* type_kind =
          ...
          all
            (List.map
               (fun (field : Syntax.Ast.record_field) ->
                 let* field_typ =
                   resolve_type env field.Syntax.Ast.field_type
                 in
                 Ok
                   {
                     Ir.field_name = field.field_name;
                     field_typ;
                     field_loc = field.field_loc;
                   })
               fields)
        in
        Ok (Ir.Record fields)
          ...
          all
            (List.map
               (fun (ctor : Syntax.Ast.variant_ctor) ->
                 let* ctor_args =
                   all (List.map (resolve_type env) ctor.Syntax.Ast.ctor_args)
                 in
                 Ok
                   {
                     Ir.ctor_name = ctor.ctor_name;
                     ctor_args;
...
let update_env_current env signature =
  { env with Env.current_module = signature }

let rec compile_module (state : compile_state) (module_name : Syntax.Ast.qname)
    : (Ir.module_, string) result =
  ...
  | None ->
      if StringSet.mem key state.visiting then
        ...
        let* () =
          List.fold_left
            (fun acc dependency ->
              ...
              Ok ())
            (Ok ())
            (Project_loader.module_dependencies ast_module)
        in
        ...
        let* module_decls, final_signature =
          check_module_decls env module_name ast_module.module_decls []
        in
        let ir_module =
          {
            Ir.module_name;
            module_path = ast_module.module_path;
            module_decls = List.rev module_decls;
            module_loc = ast_module.module_loc;
          }
        in
        state.compiled_modules <-
          StringMap.add key ir_module state.compiled_modules;
        state.compiled_signatures <-
          StringMap.add key final_signature state.compiled_signatures;
...
and check_module_decls env module_name decls acc =
  ...
  | decl :: rest -> (
      ...
      | Syntax.Ast.OpenDecl (opened, loc) ->
          ...
          check_module_decls env module_name rest
            (Ir.OpenDecl (opened, loc) :: acc)
      | Syntax.Ast.TypeDecl decl ->
          let* ir_decl = check_type_decl env module_name decl in
          let current_module =
            Env.add_type_decl env.Env.current_module ir_decl
          in
      ...
      | Syntax.Ast.LetDecl binding ->
          ...
          if binding.binding_params = [] && effectful then
            type_error binding.binding_loc
              "top-level effectful bindings are not allowed"
          else
            let current_module =
              Env.add_value env.Env.current_module ~name:binding.binding_name
                value_info
            in
            let env = update_env_current env current_module in
            check_module_decls env module_name rest
              (Ir.LetDecl ir_binding :: acc)
      | Syntax.Ast.AgentDecl callable ->
          let* ir_callable, value_info = check_callable env callable in
          let current_module =
            Env.add_value env.Env.current_module ~name:callable.callable_name
              value_info
          in
          let env = update_env_current env current_module in
          check_module_decls env module_name rest
            (Ir.AgentDecl ir_callable :: acc)
...
let build_state (program : Syntax.Ast.program) =
  let ast_modules =
    List.fold_left
      ...
      StringMap.empty program.Syntax.Ast.modules
  in
  {
    ast_modules;
...
let check ?env:_ (program : Syntax.Ast.program) : (typed_program, error) result
  ...
  let non_root_modules =
    List.filter
      (fun module_ ->
        qname_key module_.Syntax.Ast.module_name
        <> qname_key program.root_module)
...
let check_file ?(include_paths = []) (path : string) :
