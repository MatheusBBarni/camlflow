...
let parse_content_length headers =
  let rec find = function
    ...
    | header :: rest ->
        ...
        if String.length normalized >= String.length prefix
        ...
        then
          let raw =
            ...
            |> String.trim
          in
          (try Ok (int_of_string raw) with Failure _ -> Error "invalid Content-Length header")
        else find rest
  in
  find headers
...
let read_headers channel =
  let rec loop acc =
    try
      ...
      if String.equal line "" then Ok (Some (List.rev acc)) else loop (line :: acc)
    with End_of_file ->
      match acc with
      | [] -> Ok None
...
let read_message channel =
  ...
  | Some headers ->
      ...
      try Ok (Some (Yojson.Safe.from_string payload)) with
